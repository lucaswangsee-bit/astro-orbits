...
- **真实贴图**：太阳、八大行星、月球均用 NASA 影像贴图（Solar System Scope），配银河星空背景与土星环。
- **双模式**：
  - 🪐 **太阳系** —— 行星实时公转、自转、月球绕地。
  - ✨ **邻近恒星** —— 以太阳为中心，用真实赤经/赤纬/距离还原 16 颗邻近与著名恒星的三维分布（比邻星、天狼星、织女星、参宿四…），带光年参考圈。
- **天象预测**：内置引擎扫描未来 8 年，用真实轨道算出行星「冲」「大距」「相合」的精确日期，点击可跳转到当天查看。
- **时间旅行**：可加速、倒放、暂停，一键回到今天。
- **天体特征**：点击任意天体/恒星，查看明显特征与关键数据（地球特别标注了"唯一已知生命星球"、液态水、大气、磁场等）。
- **交互镜头**：拖动旋转、滚轮缩放、镜头一键对准。
...
├── styles.css          界面样式
├── assets/textures/    行星/太阳/月球贴图 + 银河背景（CC BY 4.0）
├── src/
│   ├── kepler.js       【天文引擎】JPL 轨道根数表 + 位置/轨道解算
│   ├── bodies.js       【数据层】各天体物理数据与特征描述
│   ├── stars.js        【数据层】邻近恒星真实坐标与特征
│   ├── events.js       【预测引擎】冲/大距/相合天象计算
│   └── main.js         【渲染层】Three.js 场景、双模式、时间控制、交互
└── README.md
```
...
- **行星/太阳/月球贴图、银河背景**：[Solar System Scope Textures](https://www.solarsystemscope.com/textures/) —— 授权 **CC BY 4.0**，基于 NASA 影像制作。使用本项目时请保留此署名。
- **轨道根数**：NASA JPL Solar System Dynamics（公有领域）。
- **恒星坐标**：SIMBAD / Hipparcos / Gaia 公开星表。
- **3D 引擎**：[Three.js](https://threejs.org/)（MIT）。
